...
const ChatBox = () => {
  ...
  // Load messages on userId change
  useEffect(() => {
    fetchUserMessages()
  ...
  useEffect(() => {
    if (connections?.length > 0) {
      setUser(connections.find((u) => u._id === userId) || null)
    }
  }, [connections, userId])
  ...
  useEffect(() => {
    ...
    const eventSource = new EventSource(
      `${api.defaults.baseURL}/api/message/sse/${userId}`
    )
    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="px-4">
        <div className="flex items-center gap-3 pl-5 p-1.5 bg-white w-full max-w-xl mx-auto border border-gray-200 shadow rounded-full mb-5">
          ...
          <button
            onClick={sendMessage}
            className="bg-gradient-to-br from-indigo-500 to-purple-600 hover:from-indigo-700 hover:to-purple-800 text-white p-2 rounded active:scale-95"
          >
